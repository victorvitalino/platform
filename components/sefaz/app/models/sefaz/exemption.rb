...
module Sefaz
  ...
  #  validate :virtual_validate!
    ...
    def virtual_validate!
      ...
      #POR FAVOR MELHOR ISSO UM DIA, POR FAVOR, FAZ UM LOOP HERANÇA, QUALUQER COISA
      #IGOR MARQUES DE JESUS - igor.marques@codhab.df.gov.br que fez
    ...
    def add_message_error(message)
      self.return_message  = "" if self.return_message.nil?
       ...
       xml.ArrayofImovelConstruido do
         new_exemption.each do |exemption|
           xml.imovelconstruido(id: exemption.id ) do
             xml.nometransmitente exemption.notifiers
             xml.cnpj  exemption.cnpj_notifiers.gsub('.','').gsub('-','').gsub('/','')
             xml.nomeadquirente exemption.name
             xml.cpfadquirente exemption.cpf.gsub('.','').gsub('-','')
             xml.enderecoCompleto exemption.address
             xml.inscricaoImovel exemption.realty_number      if
             xml.tipoTransacao exemption.exemption_type == "ITBI" ? 1 : 2
