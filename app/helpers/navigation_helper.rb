...
module NavigationHelper
  def nav_link_to(system_module)
    ...
    file = YAML.load_file(File.join(Rails.root,"lib/navigation/#{system_module}.yml")) rescue nil
    
    if file.present? && view_nav?(file['nav'][system_module])
      
      modules   = file["nav"]["#{system_module}"]["modules"].present?
      link_base = file["nav"]["#{system_module}"]["info"]["link"]

      if modules
        @html = <<-HTML
          <li class="dropdown #{(params[:module] == file['nav'][system_module]['info']['module']) ? 'has-active has-sub' : 'has-sub'}">
            <a aria-expanded="true" aria-haspopup="true" class="dropdown-toggle" data-toggle="dropdown" href="#{modules ? '#' : link_base}" role="button">
        ...
        @html = <<-HTML
          <li class="#{(params[:module] == file['nav'][system_module]['info']['module']) ? 'has-active has-sub' : 'has-sub'}">
            <a href="#{link_base}">
              #{file["nav"]["#{system_module}"]["info"]["label"]}
            </a>
            <ul class="#{modules ? 'dropdown-menu' : ''}"> 
        HTML
      end
      ...
      if file["nav"]["#{system_module}"]["modules"].present?
        ...
        file["nav"]["#{system_module}"]["modules"].each do |nav|
          ...
          if allow_nav?(nav[1]['code'])
            ...
            if nav[1].key?("sub") && nav[1]['sub'].count > 0
